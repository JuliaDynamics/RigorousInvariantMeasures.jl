...
function runExperiment()
    ...
    time_assembling = @elapsed begin
        ...
        D = PwMap(
            [x -> 2.5 * x, x -> 4 * x - 1, x -> 4 * x - 2, x -> 4 * x - 3],
    ...
    time_norms_fine = @elapsed norms_fine =
        finepowernormbounds(B, B_fine, D, norms; normQ_fine = normQ_fine)
    ...
    p2 = plot!(p2, B, error, w, label = "L1 error $(round(error, sigdigits=2)) / resid $(round(error_residual, sigdigits=2))")
    ...
    p3 = plot!(
        ...
        error_fine,
        w_fine,
        label = "L1 error $(round(error_fine, sigdigits=2)) / resid $(round(error_residual_fine, sigdigits=2))",
    )

    p4 = groupedbar(
        vcat(
            [time_error time_eigen time_norms time_assembling 0],
            [time_error_fine time_eigen_fine time_norms_fine time_assembling_fine time_assembling +
                                                                                  time_norms],
        ),
        bar_position = :stack,
        legend = :topleft,
