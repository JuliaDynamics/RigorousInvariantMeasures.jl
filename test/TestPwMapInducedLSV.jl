@testset "TestPwMapInducedLSV" begin
    ...
    preim_part_zero_one, label = RigorousInvariantMeasures.preimages(part_zero_one, ϕ.branches[1], 1:1024; ϵ = 0.00001, max_iter = 100)
    @test all(part_half_one[1:end-1] .∈ preim_part_zero_one)

    preim_part_half_one, label = RigorousInvariantMeasures.preimages(part_half_one, ψ.branches[1], 1:1024; ϵ = 0.00001, max_iter = 100)
