@testset "Test BasisDefinition.jl" begin
    ...
    @test_logs (:error, "Not Implemented") BasisDefinition.aux_normalized_projection_error(
        B,
    )
    ...
    @test_logs (:error, "Not Implemented") BasisDefinition.bound_weak_norm_from_linalg_norm(
        B,
    )
    ...
    @test_logs (:error, "Must be specialized") BasisDefinition.invariant_measure_strong_norm_bound(
        B,
        D,
    )
    @test_logs (:error, "Must be specialized") BasisDefinition.bound_weak_norm_abstract(
        B,
        D,
    )


    @test_logs (:error, "Must be specialized") BasisDefinition.opnormbound(
        B,
        L1,
        [
            1.0 0.0
