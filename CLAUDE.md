...
- **`DFLY`** (`src/DFLY.jl`) — Doeblin-Fortet / Lasota-Yorke inequality. `dfly(strong_norm, aux_norm, D)` returns `(A, B)` with `‖Lf‖_s ≤ A‖f‖_s + B‖f‖_aux`. The method table is the main per-basis × per-dynamic dispatch surface; missing methods return `@error "Not implemented"` (note: this logs but doesn't throw — callers without coverage silently get `nothing`. Prefer the `function projection end` extension-hook pattern for new methods, see `src/Basis/BasisDefinition.jl`).
...
## Interval API conventions (IA 1.0)

The codebase is on `IntervalArithmetic = "1"`. Several IA 0.20 idioms were intentionally removed upstream and **don't work anymore**. Don't reintroduce them, and don't try to restore them by defining `Base.<op>(::Interval, ::Interval)` shims — that pattern was explicitly rejected during migration (see commit `b295852` and `feedback_no_compat_overrides` memory).

| Don't use | Use instead | Notes |
|---|---|---|
| `Interval(a, b)` | `interval(a, b)` | 2-arg capital constructor was removed. `Interval{T}` as a *type annotation* is unchanged. |
| `Interval{T}(a, b)` | `interval(T, a, b)` | Single-arg `Interval{T}(x::Real)` still works for typed conversions. |
| `a..b` | `interval(a, b)` | The `..` operator is gone. Module-path `..` (e.g. `using ..ParentModule`) is unrelated and unaffected. |
| `x.lo` / `x.hi` | `inf(x)` / `sup(x)` | Field access is gone — `Interval` now stores `bareinterval`/`decoration`/`isguaranteed`. Watch for collisions with structs that legitimately have `lo`/`hi` fields (`HatFunction`, `HatFunctionOnTorus`). |
| `midpoint_radius(x)` | `midradius(x)` | Returns the same `(mid, rad)` tuple. |
| `a == b`, `a != b` (intervals) | `isequal_interval(a, b)`, `!isequal_interval(a, b)` | Direct `==` raises `InconclusiveBooleanOperation`. |
| `x ∈ I`, `x ∉ I` | `in_interval(x, I)`, `!in_interval(x, I)` | Same — `Base.in` falls back to `==`. |
| `a ∩ b`, `intersect(a, b)` | `intersect_interval(a, b)` | `Base.intersect` is "purposely not supported for intervals". |
| `a ∪ b`, `union(a, b)` | `hull(a, b)` | Convex hull is the rigorous union analogue. |
| `isempty(I)` | `isempty_interval(I)` | |
| `a ⊂ b`, `issubset(a, b)` | `issubset_interval(a, b)` | |
| `∅` | `emptyinterval()` (typed: `emptyinterval(typeof(x))`) | The bare symbol moved to `IntervalArithmetic.Symbols` and isn't auto-exported. |
| `isfinite(I)` | `isbounded(I)` | `isfinite` raises `InconclusiveBooleanOperation` for unbounded intervals. |
| `IntervalArithmetic.atomic(Interval{T}, x)` | `interval(T, x)` | `atomic` was removed. |
| `interval_from_midpoint_radius(m, r)` | `interval(m - r, m + r)` | The named helper is gone. |
| `IntervalArithmetic.SVector` | `using StaticArrays: SVector` | IA stopped re-exporting it. |
| `<= / ==` between `Interval` and `Real` | extract the bound: `sup(x) <= 0.5` etc. | Mixed comparisons fall back to `==` and throw. The LSV branch dispatch in `src/InducedLSV.jl:48` shows the `_ub(::Real)/_ub(::Interval)` helper pattern. |

Also note: **`@round(T, ex1, ex2)`** (used in `src/pitrig.jl`) returns a `BareInterval{T}`, not a decorated `Interval{T}`. Wrap each call in `interval(...)` to upgrade.

**Sentinel zero gotcha:** `inf(interval(0, 1))` returns `-0.0` (negative zero) under IA 1.0. `searchsortedlast([0.0, ...], -0.0)` returns 0 instead of 1, breaking `nonzero_on` and `first_overlapping`/`last_overlapping`. Every site that calls `searchsortedlast(B.p, inf(y))` or `searchsortedlast(B.p, sup(y))` needs an `nz(x) = iszero(x) ? zero(x) : x` guard — see `src/Basis/UlamBasis.jl`, `CircleHatBasis.jl`, `IntervalHatBasis.jl`, and the original `iszero(a) && (a = zero(a))` shim in `src/Preimages.jl:21`.
...
- **Directed rounding helpers**: `⊕₊`, `⊗₊`, `square_round(..., RoundUp)`, `Float64(x, RoundUp)` from `FastRounding` appear throughout — these preserve upper-bound validity when converting intervals back to floats. Don't replace them with `+`/`*`. Note: `sqrt(x, RoundUp)` was dropped in IA 1.0; use `FastRounding.sqrt_round(x, RoundUp)`.
...
`test/` has its own `Project.toml` with the test-only deps (Documenter, Symbolics, SymbolicUtils, TaylorModels, plus the stdlib pieces that individual test files explicitly `using`: LinearAlgebra, SparseArrays, Random, StaticArrays, TaylorSeries, IntervalArithmetic). `Pkg.test` uses this automatically; the legacy `[extras]` / `[targets]` block has been removed from the root `Project.toml`. `test/Manifest.toml` is gitignored.
