...
struct Fourier{T<:AbstractVector} <:Basis
	p::T
	k::Integer
end

struct Fourier2D{T<:AbstractMatrix} <:Basis
	p::T
	k::Integer
	l::Integer
end

FourierPoints(n, T) = [i/(n) for i=0:n-1]

Fourier2DPoints(n, m, T) = hcat([[(i/n,j/m) for i=0:n-1] for j=0:m-1]...)

function Fourier(n::Integer, k::Integer; T = Float64) 
    return Fourier(FourierPoints(n, T), k)
end
Base.show(io::IO, B::Fourier) = print(io, "Fourier basis on $(length(B)) points, highest frequency $(Int64(floor((length(B)-1)/2)))")

function Fourier2D(n::Integer,m::Integer,k::Integer,l::Integer; T = Float64)
	return Fourier2D(Fourier2DPoints(n, m, T), k, l)
end
Base.show(io::IO, B::Fourier2D) = print(io, "2D Fourier basis on $(size(B)) points, highest frequency ($(Int64(floor((size(B)[1]-1)/2))),$(Int64(floor((size(B)[2]-1)/2))))")

"""
Return the size of the Fourier basis
"""
Base.length(B::Fourier) = length(B.p)
Base.length(B::Fourier2D) = length(B.p)
Base.size(B::Fourier2D) = size(B.p)
...
evalFourierVector(coeff, B::Fourier) = real.(_eval_Fourier_T(length(coeff)-1, B.p)*coeff)
evalFourierDerivativeVector(coeff,  B::Fourier) =  real.(_eval_Fourier_T_derivative(length(coeff)-1, B.p)*coeff)
