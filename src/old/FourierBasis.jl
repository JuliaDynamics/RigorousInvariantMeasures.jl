...
make_dx(N, T) = [Interval{T}(i) / N for i = 0:N-1]
...
dfly(::Type{L2}, ::Type{L1}, N::GaussianNoise) =
    (0, (Interval(1) / (Interval(2) * sqrt(Interval(2)))).hi)
...
weak_by_strong_and_aux_bound(B::Fourier1D) = (0.0, 1.0)
...
function powernormboundsnoise(
    ...
    while true
        computed_norms =
            norms_of_powers_noise(B, strong_norm(B), m, Q, NK, integral_covector(B))
