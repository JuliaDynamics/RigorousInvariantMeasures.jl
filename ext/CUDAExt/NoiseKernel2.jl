...
NoiseUlam(B, k, BC!, Ext, BC) = NoiseUlam(
    B,
    k,
    BC!,
    Ext,
...
if has_cuda() && has_cuda_gpu()
    ...
    NoiseUlamCuda(B, k, BC!, Ext, BC, threads, blocks) = NoiseUlamCuda(
        B,
        k,
        BC!,
    ...
    function cuda_uniform_convolution_shared!(w_conv, w_ext, l, threads, blocks)
        ...
        @cuda threads = l cuda_left_conv!(w_conv, w_ext, l)
        @cuda threads = threads blocks = blocks shmem =
            ((threads + 2 * l) * sizeof(Float64)) cuda_central_conv_shared!(
            w_conv,
            w_ext,
            l,
        )
